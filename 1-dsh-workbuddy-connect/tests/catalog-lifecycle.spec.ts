...
describe('catalog lifecycle', () => {
  ...
  it('keeps the fallback roster and retries after a failed fetch', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn(async () => {
      attempts += 1
      // Fail the first cycle only. A cycle probes both catalog routes, so both
      // of its calls have to fail for the fetch to count as failed; the retry
      // must then recover on its own.
      return attempts <= 2
...
describe('identity changes during catalog loading', () => {
  ...
  it('cancels an old-account request and starts one for the newly selected account', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn(async (_url: unknown, init?: RequestInit) => {
      calls += 1
      // A cycle probes both catalog routes, so hold both of the first cycle's
      // calls: the cycle stays in flight until the identity change cancels it.
      if (calls <= 2) {
