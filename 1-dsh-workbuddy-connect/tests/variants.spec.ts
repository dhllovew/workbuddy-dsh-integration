...
describe('international catalog parsing', () => {
  ...
  it('parses a bare international document that omits the {code,msg,data} wrapper', async () => {
    ...
    const client = new WorkBuddyUpstreamClient({
      ...
      // Injected so the assertion never depends on this machine's Apps.
      resolveChatIdentity: async () => ({ clientVersion: '5.5.2', cliVersion: '2.137.1' }),
    })
    ...
    expect(models.map(model => model.id)).toEqual(['hy3', 'ctx-model'])
    // The App version is still resolved: it is the catalog's provenance, which
    // the card reports. It no longer shapes the User-Agent.
    expect(resolver).toHaveBeenCalledTimes(1)
    // The product document's UA gate accepts the desktop identity, so every
    // outbound path now presents it.
    for (const [, init] of request.mock.calls as unknown as [string, RequestInit][]) {
      expect((init.headers as Record<string, string>)['User-Agent'])
        .toBe('WorkBuddy/5.5.2 WorkBuddy AI/5.5.2 CLI/2.137.1')
    }
    vi.unstubAllGlobals()
  })
  ...
  it('still rejects a body that is neither wrapped nor a catalog', async () => {
    ...
    await expect(client.fetchModels({
      accessToken: 'at', refreshToken: 'rt', expiresAtMs: 0,
      domain: 'www.workbuddy.ai', uid: 'uid', source: 'desktop',
    // The route that supplies the error for this realm is the product document —
    // the international realm's authority — so its parser names the failure.
    })).rejects.toThrow(/v3 config resolved to an empty model list/)
    vi.unstubAllGlobals()
  })
})
