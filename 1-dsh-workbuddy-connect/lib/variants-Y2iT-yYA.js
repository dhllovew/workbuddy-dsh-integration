...
function billingMeterPaths(region) {
	return region === "global" ? ["/billing/meter/get-user-resource", "/v2/billing/meter/get-user-resource"] : ["/v2/billing/meter/get-user-resource"];
}
...
*/
function attributionHeaders(clientVersion) {
	return {
...
*/
function acceptLanguageFor(region) {
	return region === "global" ? "en-US" : "zh-CN";
}
/** Headers every upstream request shares. */
function commonHeaders(credential, userAgent, region) {
	return {
...
*/
function identityUserAgent(identity, region) {
	try {
...
* builds.
*/
function chatHeaders(credential, identity, region) {
	return {
...
function refreshHeaders(credential, identity, region) {
	const headers = {
		...commonHeaders(credential, identityUserAgent(identity, region), region),
...
* absent here.
*/
function billingHeaders(credential, clientVersion, region) {
	const headers = {
...
var WorkBuddyUpstreamClient = class {
	...
	*/
	async fetchModels(credential, signal) {
		...
		const [v3, enterprise] = await Promise.allSettled([this.fetchCatalogDocument(credential, "/v3/config", userAgent, region, parseV3Catalog, signal), this.fetchCatalogDocument(credential, international ? "/v2/enterprises/personal/models" : "/console/enterprises/personal/models", userAgent, region, (data) => parseModelCatalog(data, international), signal)]);
		if (v3.status === "rejected" && enterprise.status === "rejected") throw (international ? v3 : enterprise).reason;
		const v3Models = v3.status === "fulfilled" ? v3.value : [];
		const enterpriseModels = enterprise.status === "fulfilled" ? enterprise.value : [];
		const models = international ? mergeCatalogs(v3Models, enterpriseModels) : mergeCatalogs(enterpriseModels, v3Models);
		this.lastCatalog = {
	...
	*/
	async fetchCatalogDocument(credential, path, userAgent, region, parse, signal) {
		...
		const envelope = await readEnvelope(response);
		if (!response.ok || envelope.code !== 0) throw envelopeError(response.status, envelope);
		return parse(isObject(envelope.data) ? envelope.data : "models" in envelope.document || "agents" in envelope.document ? envelope.document : {});
	}
	/**
	...
	*/
	async fetchCredits(credential) {
		const region = regionOf(credential.domain);
		const identity = await this.desktopIdentity(region);
		if (region === "cn" && credential.enterpriseId !== void 0 && credential.enterpriseId !== "") return await this.fetchEnterpriseCredits(credential, identity.clientVersion);
	...
	*/
	async fetchEnterpriseCredits(credential, clientVersion) {
	...
	async postBillingMeter(credential, path, clientVersion, region, body) {
...
function isNonChatEntry(entry) {
	...
	return (Array.isArray(entry["tags"]) ? entry["tags"] : []).includes("text-to-image");
}
/** The ids of a document's non-conversation entries. */
function nonChatEntryIds(data) {
...
function cliCatalogIds(data) {
	const agents = Array.isArray(data["agents"]) ? data["agents"] : [];
	for (const agent of agents) {
		if (typeof agent !== "object" || agent === null) continue;
		const wrapped = agent;
		if (wrapped["name"] === "cli" && Array.isArray(wrapped["models"])) return wrapped["models"].filter((id) => typeof id === "string");
	}
}
/**
...
* enterprise document.
*/
function indexCatalogModels(data, international) {
	const rawModels = Array.isArray(data["models"]) ? data["models"] : [];
...
function parseV3Catalog(data) {
	...
	} else models = cliIds.map((id) => byId.get(id)).filter((model) => model !== void 0);
