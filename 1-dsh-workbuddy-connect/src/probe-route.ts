...
export interface WorkBuddyProbeRouteOptions {
  ...
  /**
   * Persist this variant's model selection.
   *
   * `models === undefined` clears the field, restoring "expose every model";
   * an empty array records a deliberate empty selection.
   */
  setSelectedModels?: (models: readonly string[] | undefined) => Promise<{ state: string; reason?: string }>
  /**
...
function parseAction(text: string): WorkBuddyProbeAction | undefined {
  ...
  if (action === 'set-selected-models') {
    ...
    // Present-but-null clears the field (restore "no preference"); absent is a
    // malformed request, not the same instruction.
    if (models === null) return { action: 'set-selected-models', selectedModels: null }
    if (!Array.isArray(models)) return undefined
    // Non-string entries are rejected rather than coerced: this value is
    // written straight into the settings document, and a coerced number would
    // be stored as a model id nobody can match.
    if (!models.every(entry => typeof entry === 'string')) return undefined
    return { action: 'set-selected-models', selectedModels: models as string[] }
  }
  if (action === 'probe') {
  ...
  return async (req, res) => {
    ...
    try {
      ...
      if (action.action === 'set-selected-models') {
        if (deps.setSelectedModels === undefined) {
          json(res, 404, { error: 'model-selection-not-supported' })
          return
        }
        json(res, 200, await deps.setSelectedModels(action.selectedModels === null ? undefined : action.selectedModels))
        return
      }
