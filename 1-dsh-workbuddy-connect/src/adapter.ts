...
export function createWorkBuddyAdapter(options: WorkBuddyAdapterOptions): WorkBuddyAdapter {
  ...
  const buildModels = (): Model<Api>[] => {
    ...
    // `available()` rather than `current()`: the collection is the routing
    // surface, so it carries the whole catalog and ignores the selection.
...
class WorkBuddyPiAiAdapter extends PiAiAdapter {
   ...
   */
  private anyInfoFor(model: string): WorkBuddyModelInfo | undefined {
  ...
  override async resolveModel(provider: string, model: string, signal?: AbortSignal): Promise<LlmResolvedModelInfo> {
    ...
    // Deliberately `anyInfoFor`, not `infoFor`: the selection filters what the
    // picker lists, never what this route can resolve.
