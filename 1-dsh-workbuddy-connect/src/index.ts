...
import type { WorkBuddyWebCatalog, WorkBuddyWebModelSelection, WorkBuddyWebProbeSection } from './status-paths.ts'
 ...
 */
function selectionSection(runtime: VariantRuntime): WorkBuddyWebModelSelection {
  const selection = runtime.catalog.selection()
  const unconfigured = selection === undefined
  const chosen = selection === undefined ? undefined : new Set(selection)
  return {
    models: runtime.catalog.available().map(info => ({
      id: info.id,
      name: info.name,
      // An unconfigured selection shows everything, which is what the picker
      // actually does; `unconfigured` carries the distinction for the copy.
...
export function apply(ctx: Context, config: Config): void {
   ...
   */
  let setSelectedModels: ((variantId: string, models: readonly string[] | undefined) => Promise<{ state: string; reason?: string }>) | undefined
  /**
    ...
    for (const runtime of runtimes) {
      ...
      registerWorkBuddyProbeRoute(webCtx, {
        ...
        setSelectedModels: async models => {
          if (setSelectedModels === undefined) return { state: 'failed', reason: 'settings are unavailable' }
  ...
  ctx.inject(['settings'], settingsCtx => {
    ...
    const merged = (): Config => ({
      ...
      ...sources.cn().selectedModels === undefined ? {} : { selectedModels: sources.cn().selectedModels },
      ...
      ...sources.ai().selectedModelsAI === undefined ? {} : { selectedModelsAI: sources.ai().selectedModelsAI },
    })
    /**
     ...
     */
    const applySelection = (next: Config): void => {
      for (const runtime of runtimes) {
        const selection = runtime.variant.id === CN_VARIANT.id
          ? next.selectedModels
          : next.selectedModelsAI
    ...
    settingsCtx.settings.installSection(ctx, WORKBUDDY_SETTINGS_NS, CN_SECTION, config, {
      setSource(source) { sources.cn = source as () => Config; current = merged; applySelection(merged()) },
      onChange: repointStores,
    })
    settingsCtx.settings.installSection(ctx, WORKBUDDY_AI_SETTINGS_NS, AI_SECTION, config, {
      setSource(source) { sources.ai = source as () => Config; current = merged; applySelection(merged()) },
      onChange: repointStores,
    })
    setMaximumContextWindow = async enabled => {
